  ...
  <div>
    <el-dialog width="75%" title="團隊獎勵" :before-close="handleClose" :visible.sync="dialogFormVisible">
      <el-form ref="searchForm" size="mini" :inline="true" :model="searchForm">
        ...
        <el-form-item label="獎勵類型：">
          <el-select v-model="searchForm.type" placeholder="請選擇">
            <el-option
              v-for="item in options.fenhong_income_type_enum"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            />
      ...
      >
        ...
        <el-table-column prop="type_str" label="獎勵類型" min-width="100" />
...
<script>
import { fenhongIncomeList } from '@/api/adifi'
export default {
  name: '',
  props: {
    options: {
  ...
  methods: {
    ...
    async getData() {
      const { data } = await fenhongIncomeList(this.searchForm)
