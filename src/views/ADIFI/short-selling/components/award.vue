<template>
  <div>
    <el-dialog width="75%" title="團隊獎勵" :before-close="handleClose" :visible.sync="dialogFormVisible">
      ...
      >
        <el-table-column show-overflow-tooltip prop="relation_addr" label="錢包地址" min-width="160" />
        <el-table-column prop="level_str" label="等級" min-width="80" />
        <el-table-column show-overflow-tooltip prop="tj_relation_addr" label="推薦人" min-width="160" />
...
<script>
import { kongIncomeList } from '@/api/adifi'
export default {
  name: '',
  data() {
    return {
  ...
  methods: {
    ...
    async getData() {
      const { data } = await kongIncomeList(this.searchForm)
