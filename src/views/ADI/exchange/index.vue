<template>
  <div>
    <el-card shadow="never" class="page-card">
      <SearchForm ref="searchForm" :options="options" :search-config="searchConfig" @search="onSearch" @init="onInit" />
      <div>
        <TablePackage
          v-loading="loading"
          :data="tableData"
          :size="$size"
          border
          storage-name="adi-equity"
          style="width: 100%"
          :header-cell-style="{ 'text-align': 'center','background':'#fafafa' }"
          :cell-style="{ 'text-align': 'center' }"
        >
          <template #btnContentLeft />
          <template #btnContentRight>
            <el-tooltip class="item table-top-btn" effect="dark" content="刷新列表" placement="top">
              <el-button size="mini" circle icon="el-icon-refresh-right" @click="getData" />
            </el-tooltip>
          </template>
          <el-table-column fixed show-overflow-tooltip prop="relation_addr" label="關聯地址" min-width="150" />
          <el-table-column prop="order_price" label="兌換數量" min-width="150" />
          <el-table-column prop="station_str" label="兌換狀態" min-width="150">
            <template slot-scope="scope">
              <el-button type="text" size="mini" @click="stationLog(scope.row)">{{ scope.row.station_str }}</el-button>
            </template>
          </el-table-column>
          <el-table-column prop="created_at" label="創建時間" min-width="160" />
        </TablePackage>
        <div class="footerpage">
          <el-pagination
            background
            :current-page="searchForm.page"
            :page-size="searchForm.page_size"
            layout="total,  prev, pager, next, jumper"
            :total="searchForm.total"
            @current-change="handleCurrentChange"
          />
        </div>
      </div>
    </el-card>
    <Logs ref="Logs" />
  </div>
</template>

<script>
import TablePackage from '@/components/TablePackage'
import SearchForm from '@/components/SearchForm'
import { pageMixins } from '@/mixins/page'
import Logs from './components/logs'
import { adi2dotExchangeList, adi2dotExchangeEnums } from '@/api/adi.js'
export default {
  name: 'ADIExchange', // 兌換記錄
  components: { TablePackage, SearchForm, Logs },
  mixins: [pageMixins],
  data() {
    return {
      loading: false,
      options: {
        station_enum: []
      },
      searchConfig: [
        {
          key: 'relation_addr',
          type: 'input',
          label: '關聯地址',
          value: ''
        },
        {
          key: 'station',
          type: 'select',
          label: '兌換狀態',
          value: ''
        },
        {
          key: 'create_time',
          type: 'datetimerange',
          label: '創建時間',
          is_simple: true,
          value: ''
        }
      ],
      tableData: []
    }
  },
  computed: {
  },
  created() {
    this.getEnum()
  },
  methods: {
    async getEnum() {
      const { data } = await adi2dotExchangeEnums()
      this.options.station_enum = data.exchange_state
      // console.log(data, '8888888')
    },
    async getData() {
      this.loading = true
      const { data } = await adi2dotExchangeList(this.getSearchOptions())
      this.loading = false
      this.tableData = data.data
      this.searchForm.total = data.total
    },
    stationLog(params) {
      this.$refs.Logs.dialogFormVisible = true
      this.$refs.Logs.tableData = params.process
    }
  }
}
</script>
<style lang='scss'  scoped>

</style>
