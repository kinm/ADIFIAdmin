<template>
  <div v-if="dialogFormVisible">
    <el-dialog width="65%" title="團隊詳情" :visible.sync="dialogFormVisible">
      <el-card>
        <el-descriptions direction="vertical" size="mini" :column="5" border>
          <el-descriptions-item label="用戶ID">{{ userInfo.uid }}</el-descriptions-item>
          <el-descriptions-item label="ADI個人業績">{{ userInfo.accu_part }}</el-descriptions-item>
          <el-descriptions-item label="ADI團隊業績">{{ userInfo.team_yeji }}</el-descriptions-item>
          <el-descriptions-item label="APE個人業績">{{ userInfo.old_team }}</el-descriptions-item>
          <el-descriptions-item label="APE團隊業績">{{ userInfo.old_part }}</el-descriptions-item>
        </el-descriptions>
...
<script>
import { userAdiGroup, getDirectById } from '@/api/adi'
export default {
  name: '',
  data() {
    return {
  ...
  methods: {
    async getData(params) {
      const { data } = await userAdiGroup({ id: params.id })
    ...
    loadLevelNode(node, resolve) {
      setTimeout(async() => {
        if (node.level === 0) {
          const { data } = await userAdiGroup({ id: this.userId })
