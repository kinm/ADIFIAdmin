    ...
    <el-card shadow="never" class="page-card">
      ...
      <div>
        <TablePackage
          ...
          :size="$size"
          border
          storage-name="adi-collect"
          style="width: 100%"
          :header-cell-style="{ 'text-align': 'center','background':'#fafafa' }"
          :cell-style="{ 'text-align': 'center' }"
        >
          <template #btnContentLeft>
            <el-button v-setButtonPower="'adi-collect-set'" type="primary" :size="$s_size" @click="handlebatchClick('set')">領取設置</el-button>
            <!-- <el-button v-setButtonPower="'adi-collect-auto'" type="primary" :size="$s_size" @click="handlebatchClick('auto')">自動審核</el-button> -->
          </template>
          ...
          <el-table-column fixed show-overflow-tooltip prop="order_sn" label="領取編號" min-width="150" />
          <el-table-column fixed show-overflow-tooltip prop="relation_addr" label="關聯地址" min-width="150" />
          <el-table-column prop="base" label="領取數量" min-width="150" />
          ...
          <!-- <el-table-column prop="state_str" label="狀態" min-width="150">
          ...
          <!-- <el-table-column label="操作" class="operate-table" width="80" fixed="right">
            <template slot-scope="scope">
              <el-dropdown v-setButtonPower="'adi-collect-audit'" trigger="click" size="small" placement="bottom-end" :hide-on-click="false" @command="(e)=>{ handleClick(e,scope.row, scope.$index) }">
...
import { xyBalanceWithdrawEnums, xyBalanceWithdrawList, xyBalanceWithdrawPass, xyBalanceWithdrawReject } from '@/api/adi'
export default {
  name: 'ADICollect', // 領取記錄
  ...
  methods: {
    async getEnum() {
      const { data } = await xyBalanceWithdrawEnums()
      this.options.state_enum = data.xywithdraw_state
      this.options.type_enum = data.xywithdraw_type
      this.options.unit_enum = data.xywithdraw_commission_type
    },
    async getData() {
      this.loading = true
      const { data } = await xyBalanceWithdrawList(this.getSearchOptions())
      this.loading = false
    ...
    setRefuse(params) {
      this.$prompt('', '拒絕原因', {
        ...
        await xyBalanceWithdrawReject({ id: params.id, reason: value })
    ...
    async setPass(data) {
      this.$prompt('', '请输入HASH', {
        ...
        await xyBalanceWithdrawPass({ id: data.id, hash: value })
        elMessage(this, '操作成功！', 'success')
        this.getData()
      }).catch(() => {})
    }
