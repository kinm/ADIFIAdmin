<template>
  <div>
    <el-card shadow="never" class="page-card">
      <SearchForm ref="searchForm" :options="options" :search-config="searchConfig" @search="onSearch" @init="onInit" />
      <div>
        <TablePackage
          v-loading="loading"
          :data="tableData"
          :size="$size"
          border
          storage-name="adi-harvest"
          style="width: 100%"
          :header-cell-style="{ 'text-align': 'center','background':'#fafafa' }"
          :cell-style="{ 'text-align': 'center' }"
        >
          <template #btnContentLeft />
          <template #btnContentRight>
            <el-tooltip class="item table-top-btn" effect="dark" content="刷新列表" placement="top">
              <el-button size="mini" circle icon="el-icon-refresh-right" @click="getData" />
            </el-tooltip>
          </template>
          <el-table-column fixed show-overflow-tooltip prop="relation_addr" label="關聯地址" min-width="150" />
          <el-table-column prop="num" label="領取數量" min-width="150" />
          <el-table-column prop="state_str" label="領取狀態" min-width="150">
            <template slot-scope="scope">
              <div>
                {{ scope.row.state_str }}
              </div>
            </template>
          </el-table-column>
          <el-table-column show-overflow-tooltip prop="txid" label="hash" min-width="150" />
          <el-table-column prop="created_at" label="領取時間" min-width="160" />
        </TablePackage>
        <div class="footerpage">
          <el-pagination
            background
            :current-page="searchForm.page"
            :page-size="searchForm.page_size"
            layout="total,  prev, pager, next, jumper"
            :total="searchForm.total"
            @current-change="handleCurrentChange"
          />
        </div>
      </div>
    </el-card>
  </div>
</template>

<script>
import TablePackage from '@/components/TablePackage'
import SearchForm from '@/components/SearchForm'
import { pageMixins } from '@/mixins/page'
import { ipoAwardsList, awardsEnum } from '@/api/adi'
export default {
  name: 'ADIHarvest', // 私募领取
  components: { TablePackage, SearchForm },
  mixins: [pageMixins],
  data() {
    return {
      loading: false,
      options: {
        state_enum: []
      },
      searchConfig: [
        {
          key: 'relation_addr',
          type: 'input',
          label: '關聯地址',
          value: ''
        },
        {
          key: 'num',
          type: 'interval',
          label: '領取數量',
          value: ''
        },
        {
          key: 'state',
          type: 'select',
          label: '領取狀態',
          value: ''
        },
        {
          key: 'create_time',
          type: 'datetimerange',
          label: '领取時間',
          is_simple: true,
          value: ''
        },
        {
          key: 'txid',
          type: 'input',
          label: 'hash',
          value: ''
        }
      ],
      tableData: []
    }
  },
  computed: {
  },
  created() {
    this.getEnum()
  },
  methods: {
    async getEnum() {
      const { data } = await awardsEnum()
      this.options.state_enum = data.ipowithdraw_state
    },
    async getData() {
      this.loading = true
      const { data } = await ipoAwardsList(this.getSearchOptions())
      this.loading = false
      this.tableData = data.data
      this.searchForm.total = data.total
    }
  }
}
</script>
<style lang='scss'  scoped>

</style>
