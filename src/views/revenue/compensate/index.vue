    ...
    <el-card shadow="never" class="page-card">
      ...
      <div>
        <TablePackage
          ...
          :size="$size"
          border
          storage-name="revenue-compensate"
          style="width: 100%"
        ...
        >
          <template #btnContentLeft>
            ...
            <el-button v-setButtonPower="'revenue-compensate-export'" type="primary" :size="$s_size" @click="handlebatchClick('export')">導出</el-button>
          </template>
          ...
          <el-table-column prop="balance" label="永動補償池（APE）" min-width="140" />
          ...
          <!-- <el-table-column show-overflow-tooltip label="備註" min-width="160">
            <template slot-scope="scope">
              <div class="table-omit">
                <span v-if="buttonRoles.some((el) => el === 'revenue-compensate-remark')" class="icon-edit" @click="editRemarks(scope.row)"><i class="el-icon-edit" /></span>
                {{ scope.row.remark }}
              </div>
            </template>
          </el-table-column> -->
...
import { incomeList4, incomeExportDobu } from '@/api/revenue'
export default {
  name: 'RevenueCompensate', // 永動補償
  ...
  methods: {
    async getData() {
      this.loading = true
      const { data } = await incomeList4(this.getSearchOptions())
      this.loading = false
    ...
    async handlebatchClick(type) {
      if (type === 'export') {
        ...
        delete params.total
        const { data } = await incomeExportDobu(params)
        var oReq = new XMLHttpRequest()
        oReq.open('GET', data.excel, true)
        oReq.responseType = 'blob'
        oReq.onload = () => {
          var file = new Blob([oReq.response], {
            type: 'application/vnd.ms-excel' || 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
          })
          FileSaver.saveAs(file, decodeURI(data.excel.split('/')[data.excel.split('/').length - 1]))
        }
        oReq.send()
      }
    },
