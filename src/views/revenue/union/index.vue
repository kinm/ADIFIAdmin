      ...
      <div>
        ...
        >
          <template #btnContentLeft>
            ...
            <el-button v-setButtonPower="'revenue-union-export'" type="primary" :size="$s_size" @click="handlebatchClick('export')">導出</el-button>
          </template>
          ...
          <el-table-column prop="role_str" label="聯盟委員" min-width="120" />
          ...
          <!-- <el-table-column show-overflow-tooltip label="備註" min-width="160">
            <template slot-scope="scope">
              <div class="table-omit">
                <span v-if="buttonRoles.some((el) => el === 'revenue-union-remark')" class="icon-edit" @click="editRemarks(scope.row)"><i class="el-icon-edit" /></span>
                {{ scope.row.remark }}
              </div>
            </template>
          </el-table-column> -->
...
import { incomeList5, incomeEnums, incomeExportDounion } from '@/api/revenue'
export default {
  name: 'RevenueUnion', // 聯盟收益
  ...
  methods: {
    ...
    async getData() {
      this.loading = true
      const { data } = await incomeList5(this.getSearchOptions())
      this.loading = false
    ...
    async handlebatchClick(type) {
      if (type === 'export') {
        ...
        delete params.total
        const { data } = await incomeExportDounion(params)
